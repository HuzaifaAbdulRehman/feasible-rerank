...
def main() -> None:
    ...
    ap.add_argument("--n-items", type=int, default=22)
    ...
    print(f"enumerating C({args.n_items}, {args.k}) = "
          f"{comb(args.n_items, args.k):,} subsets per instance, "
    ...
    for repeat in range(args.repeats):
        ...
        rows.append({"repeat": repeat, "method": "random_feasible",
                     "energy": random_energy, "optimum": optimum,
                     "random": random_energy, "feasible": True, "optimal": False})

        print(f"  instance {repeat:>2}  optimum {optimum:+.6f}   "
    ...
    print(f"\n{'method':<18}{'gap to optimal':>16}{'% recovered':>14}"
          f"{'exactly optimal':>17}{'feasible':>10}")
    order = ["greedy_topk", "mmr", "quota_mmr", "qubo_sa", "qubo_sb",
